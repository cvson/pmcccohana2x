{
    ...
    //sum of background
    TH1F* hsig = (TH1F*)hmcen[1][1]->Clone("hsignal");
    TH1F* hbkg = (TH1F*)hmcen[1][2]->Clone("hbackground");
    for (Int_t iint=3; iint<NINTERACTION; ++iint) {
        hbkg->Add(hmcen[1][iint]);
    }
    ...
    new TCanvas;
    hoptimize->GetXaxis()->SetTitle("Cut values on discriminant parameter");
    ...
    hoptimize->Draw("hist");
    gPad->Print("plots/ana1_pidcoh_opt.eps");
    //hbkg->SetLineColor(2);
    //hbkg->Draw("hist same");
