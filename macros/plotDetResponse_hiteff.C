{
    ...
    for (int isam=0; isam<NSAMPLE; ++isam) {
        ...
        gPad->Print(Form("plots/detresponse_hiteff_all_sam%d_compSyst.eps",isam));
    ...
    for (Int_t ibin=0; ibin<NBINCCQE; ibin++) {
        ...
        leg0->Draw("same");
        gPad->Modified();
        gPad->Print(Form("plots/detresponse_hiteff_bin%d_compSyst.eps",ibin));
    ...
    TFile *popfile = new TFile("../inputs/detresponse_hiteff_uncertainty.root","RECREATE");
    for (int isam=0; isam<NSAMPLE; ++isam) {
        for (Int_t ibin=0; ibin<NBINCCQE; ibin++) {
        ...
        pUncertainty[isam]->Write(Form("detresponse_hiteff_uncertainty_sam%d",isam));
    }
    popfile->Write();
    popfile->Close();
