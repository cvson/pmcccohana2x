...
// Thu Aug  6 23:07:31 2015 by ROOT version 5.34/32
// from TTree tree/tree
// found on file: ../../syst/frKikawa/MC/pm/pe/merged_ccqe_0.root
//////////////////////////////////////////////////////////

#ifndef baseTreeDet_h
#define baseTreeDet_h
...
class baseTreeDet {
    ...
    virtual void     Loop();
    virtual void     MakeHitEfficiencySyst(TObjArray* paFile, TObjArray* paHitEffFile, char* treeName, TString opFile, std::vector<std::pair <double,double> > v_pedges, std::vector<std::pair <double,double> > v_cthedges);
    virtual void     MakeMPPCNoiseSyt(TObjArray* paFile, char* treeName, TString opFile, std::vector<std::pair <double,double> > v_pedges, std::vector<std::pair <double,double> > v_cthedges);
    virtual void     MakeResponseFunction(TObjArray* paFile, char* treeName, TString opFile, std::vector<std::pair <double,double> > v_pedges, std::vector<std::pair <double,double> > v_cthedges);
    virtual void     MakeResponseFunctionReco(TObjArray* paFile, TObjArray* paFiledata, char* treeName, TString opFile, std::vector<std::pair <double,double> > v_pedges, std::vector<std::pair <double,double> > v_cthedges);
...
#ifdef baseTreeDet_cxx
baseTreeDet::baseTreeDet(TTree *tree) : fChain(0)
{
    // if parameter tree is not specified (or zero), connect the file
    // used to generate this class and read the Tree.
    if (tree == 0) {
        TFile *f = (TFile*)gROOT->GetListOfFiles()->FindObject("../../systfsi/frKikawa/MC/pm/pe/merged_ccqe_0.root");
        if (!f || !f->IsOpen()) {
            f = new TFile("../../systfsi/frKikawa/MC/pm/pe/merged_ccqe_0.root");
        }
...
baseTreeDet::~baseTreeDet()
{
    if (!fChain) return;
    delete fChain->GetCurrentFile();
}

Int_t baseTreeDet::GetEntry(Long64_t entry)
{
    // Read contents of entry.
    if (!fChain) return 0;
    return fChain->GetEntry(entry);
}
Long64_t baseTreeDet::LoadTree(Long64_t entry)
{
...
void baseTreeDet::Init(TTree *tree)
{
...
void baseTreeDet::Show(Long64_t entry)
{
...
Int_t baseTreeDet::Cut(Long64_t entry)
{
...
Int_t baseTreeDet::CutPreselection(Long64_t entry)
{
    //REMOVING side-exiting sample
    //if (Ntrack==2 && pang<90 && !(mufc==0 && pene<9) && mumucl>0.6) return 1;//newchange
    if (Ntrack==2 && pang<90 && !(mufc==0 && pene<9)) return 1;//isolate ccqe
    else return -1;
}

Int_t baseTreeDet::CutTOPO(Long64_t entry, Int_t topotype){
...
Int_t    baseTreeDet::CutSignalR(Long64_t entry){
...
Int_t    baseTreeDet::CutCR1(Long64_t entry){
...
Int_t    baseTreeDet::CutCR23Comb(Long64_t entry){
...
Int_t    baseTreeDet::CutCR23QEIsolated(Long64_t entry){
    //if (CutCR23Comb(entry)>0 && pidccqe>-0.075 ) {
    if (CutPreselection(entry)>0 && pidfsi<-0.05 && pidfsi1pres<0.04) {//final
        return 1;
    }
    else return -1;
}
Int_t    baseTreeDet::CutCR23QEIsolatedLoQsq(Long64_t entry){
...
Int_t    baseTreeDet::CutCR23QEIsolatedHiQsq(Long64_t entry){
    //if (CutCR23QEIsolated(entry)>0 && GetQsq(GetPrec(range)*1.e3,muang_t)/1.e6>0.05) {
    if (CutCR23QEIsolated(entry)>0 && GetQsq(GetPrec(range)*1.e3,muang_t)/1.e6>0.05) {
        return 1;
    }
...
Int_t    baseTreeDet::CutCR23QEFreed(Long64_t entry){
    //if (CutCR23Comb(entry)>0 && pidccqe<-0.075 ) {
...
Int_t    baseTreeDet::CutCR23QEFreedLowVeract(Long64_t entry){
...
Int_t    baseTreeDet::CutCR23QEFreedHighVeract(Long64_t entry){
...
Int_t    baseTreeDet::CutCR23QEFreedHighVeractHiResPID(Long64_t entry){
    if (CutCR23QEFreedHighVeract(entry)>0 && pid1pres>0) {
        return 1;
    }
    else return -1;
}
Int_t    baseTreeDet::CutCR23QEFreedHighVeractLoResPID(Long64_t entry){
    if (CutCR23QEFreedHighVeract(entry)>0 && pid1pres<0) {
        return 1;
    }
    else return -1;
...
Int_t    baseTreeDet::CutMore2Track(Long64_t entry){
...
//Temporary
Int_t    baseTreeDet::CutMore2TrackHiResPID(Long64_t entry){
...
Int_t    baseTreeDet::CutMore2TrackLoResPID(Long64_t entry){
    //if (CutMore2Track(entry)>0 && pid<-0.125 && veract<475) {
...
Int_t    baseTreeDet::CutMore2TrackAllElse(Long64_t entry){
    //if (CutMore2Track(entry)>0 && pid<-0.125 && veract>475) {
    if (CutMore2Track(entry)>0 && pidfsi<-0.05 && veract*7.6634e-2>60 && pidfsi1pres>0.07) {
        return 1;
    }
...
Int_t    baseTreeDet::CutCCQE1track(Long64_t entry){
    if (Ntrack==1 && mumucl>0.6 && Ningtrack==1 && !(mufc==0 && pene<9)) {
        return 1;
    }
    else return -1;
}
Int_t    baseTreeDet::CutCCQE1trackLoQsq(Long64_t entry){
    if (CutCCQE1track(entry)>0 && GetQsq(GetPrec(range)*1.e3,muang_t)/1.e6<0.05) {
        return 1;
    }
    else return -1;
}
Int_t    baseTreeDet::CutCCQE1trackHiQsq(Long64_t entry){
    if (CutCCQE1track(entry)>0 && GetQsq(GetPrec(range)*1.e3,muang_t)/1.e6>0.05) {
        return 1;
    }
    else return -1;
...
Int_t baseTreeDet::CutbyID(Long64_t entry, Int_t myid)
{
    //all
    if (myid==0) return 1;
...
Int_t baseTreeDet::CutSamplebyID(Long64_t entry, Int_t myid)
{
...
Int_t baseTreeDet::CutInteractionbyID(Long64_t entry, Int_t myid)
{
    //all
    if (myid==0) return 1;
...
Int_t baseTreeDet::CutInteractionbyIDnew(Long64_t entry, Int_t myid)
{
...
//data 1, 0-mc
Int_t    baseTreeDet::GetEventType(Long64_t entry){
...
Int_t    baseTreeDet::GetInteractionType(Long64_t entry){
...
double baseTreeDet::GetPrec(double rangeval)
{
    double prec = rangeval*0.0114127 + 0.230608;
    return prec;
}

double baseTreeDet::GetErec(double p, double theta)
{
    //Needed quantities
    double Eb=27.;
    double Mn=939.56;
...
double baseTreeDet::GetQsq(double p, double theta)
{
    //Needed quantities
    double Mmu=105.658;
    double Emu=TMath::Sqrt(p*p+Mmu*Mmu);
...
double baseTreeDet::GetXbjorken(double p, double theta)
{
    //Needed quantities
    double Mn=939.56;
    double Mp=938.272;
...
double baseTreeDet::GetInelasticity(double p, double theta)
{
    //Needed quantities
    double Mmu=105.658;
    double Emu=TMath::Sqrt(p*p+Mmu*Mmu);
