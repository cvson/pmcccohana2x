...
baseTree::baseTree(TTree *tree) : fChain(0)
{
    ...
    if (tree == 0) {
        TFile *f = (TFile*)gROOT->GetListOfFiles()->FindObject("../../datafsicornorange/datamc_merged_ccqe_addpidFFnew_addnue.root");
        if (!f || !f->IsOpen()) {
            f = new TFile("../../datafsicornorange/datamc_merged_ccqe_addpidFFnew_addnue.root");
        }
...
Int_t    baseTree::CutSignalR(Long64_t entry){
    if (CutPreselection(entry)>0 && pidfsi>0.08 && veract*7.6634e-2<36 && pidfsi1prescor>-0.05 && pidfsiccqecor<0.0 && GetQsq(GetPrec(range)*1.e3,muang_t)/1.e6<0.15) {//opt1, opt2, final
        return 1;
    }
    else return -1;
}
Int_t    baseTree::CutCR1(Long64_t entry){
    ...
    if (CutPreselection(entry)>0 && !(pidfsi>0.08 && veract*7.6634e-2<36) && veract*7.6634e-2<50  && pidfsi1prescor>-0.05 && pidfsiccqecor<0.0 && GetQsq(GetPrec(range)*1.e3,muang_t)/1.e6<0.15) {//opt9, final
...
Int_t    baseTree::CutCR23Comb(Long64_t entry){
    if (Ntrack>2 && pang_t<90 && veract*7.6634e-2<50  && pidfsi1prescor>-0.05 && pidfsiccqecor<0.0 && !(mufc==0 && pene<9) && GetQsq(GetPrec(range)*1.e3,muang_t)/1.e6<0.15) {//final
...
Int_t    baseTree::CutMore2TrackSignal(Long64_t entry){
    if (CutMore2Track(entry)>0 && pidfsi>-0.05 && veract*7.6634e-2<60) {
        return 1;
    }
    else return -1;
}
...
Int_t baseTree::CutTopologybyID(Long64_t entry, Int_t topoID)
{
    ...
    //plot for include pid cut only
        ...
        if (CutPreselection(entry)>0 && pidfsi>0.08) return 1;//with preselection and pid
        else return -1;
    }
    //SIG
    else if (topoID==3) {
    ...
    //test sideband 2trk optimized
    else if (topoID==9) {
        if (CutPreselection(entry)>0 && pidfsi<=0.08 && pidfsi1prescor>-0.05 && pidfsiccqecor<0.0) return 1;
        else return -1;
    }
    //antipid >2 trk
    else if (topoID==10) {
        if (Ntrack>2 && pidfsi<=0.08 && !(mufc==0 && pene<9)) return 1;
        else return -1;
    }
    //one muon and one pion track all
    else if (topoID==11) {
        if (Ntrack>2 && pidfsi<=0.08 && pang_t<90 && !(mufc==0 && pene<9)) return 1;
        else return -1;
    }
    
    //one muon and one pion track all
    else if (topoID==12) {
        if (Ntrack>2 && pidfsi<=0.08 && pang_t<90  && pidfsi1prescor>-0.05 && pidfsiccqecor<0.0 && !(mufc==0 && pene<9)) return 1;
        else return -1;
    }
    //one muon and one pion stopped muon
    else if (topoID==13) {
        if (Ntrack>=2 && pidfsi<=0.08 && pang_t<90 && !(mufc==0 && pene<9)) return 1;
        else return -1;
    }
    //one muon and one proton track all
    else if (topoID==14) {
        if (Ntrack>=2 && pidfsi<=0.08 && pang_t<90 && !(mufc==0 && pene<9) && pidfsi1prescor>-0.05 && pidfsiccqecor<0.0) return 1;
        else return -1;
    }
    
    //one muon and one proton track all
    else if (topoID==15) {
        if ((CutCR1(entry)>0 || CutCR23Comb(entry)>0) && pidfsi>-0.15 && veract*7.6634e-2<60 ) return 1;
        else return -1;
    }
    //one muon and one proton stopped
    else if (topoID==16) {
        if (CutCR23Comb(entry)>0 && pidfsi>-0.15 && veract*7.6634e-2<60) return 1;
        else return -1;
    }
    //one proton track all
    else if (topoID==17) {
        if (CutCR1(entry)>0 && pidfsi>-0.15 && veract*7.6634e-2<60) return 1;
        else return -1;
    }
    
    //one proton track all
    else if (topoID==18) {
        if ((CutCR1(entry)>0 || CutCR23Comb(entry)>0) && pidfsi>-0.15 && veract*7.6634e-2<100 ) return 1;
        else return -1;
    }
    //one proton stopped
    else if (topoID==19) {
        if (CutCR23Comb(entry)>0 && pidfsi>-0.15 && veract*7.6634e-2<100) return 1;
        else return -1;
    }

    //one muon and one pion track all
    else if (topoID==20) {
        //if (Ntrack==2 && pang<90 && pidfsi<=0.08 &&  mumucl>0.6 && pmucl>0.25) return 1;
        if (CutCR1(entry)>0 && pidfsi>-0.15 && veract*7.6634e-2<100) return 1;
        else return -1;
    }
    
    //kikawa pass 
    else if (topoID==21) {
        if ((CutCR1(entry)>0 || CutCR23Comb(entry)>0) && pidfsi>-0.15 ) return 1;
        else return -1;
    }
    //pass mva but not kiwawa
    else if (topoID==22) {
        if (CutCR23Comb(entry)>0 && pidfsi>-0.15) return 1;
        else return -1;
    }
    
    //pass kikawa but not mva
    else if (topoID==23) {
        if (CutCR1(entry)>0 && pidfsi>-0.15) return 1;
        else return -1;
    }
...
Int_t baseTree::CutTopologyFSIbyID(Long64_t entry, Int_t topoID)
{
    ...
    //plot for include pid cut only
        ...
        if (CutPreselection(entry)>0 && pidfsi>0.08) return 1;//CCQE with one track
        else return -1;
    }
    //SIG
    else if (topoID==2) {
        if (CutSignalR(entry)>0) return 1;
        //if (CutPreselection(entry)>0 && pidfsi>0.08 && veract*7.6634e-2<36) return 1;//selected
        else return -1;
    }
    //CRI
    else if (topoID==3) {
