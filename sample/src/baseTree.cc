...
void baseTree::GenerateDetSyst(TString ipFile, char* treeName, TString opFile, std::vector<std::pair <double,double> > v_pedges, std::vector<std::pair <double,double> > v_cthedges){
    ...
    //Fill TGraph
    //timing
    TGraph *pgrccqe_timing[NSAMPLE][20];
    double nevtbyBin_timing[3];
    double xsigma_timing[3]={0.5,1.5,2.5};
    
    //FV
    TGraph *pgrccqemc_fv[NSAMPLE][20];
    TGraph *pgrccqedt_fv[NSAMPLE][20];
    TGraph *pgrccqesub_fv[NSAMPLE][20];
    ...
    //veto
    TGraph *pgrccqemc_veto[NSAMPLE][20];
    TGraph *pgrccqedt_veto[NSAMPLE][20];
    TGraph *pgrccqesub_veto[NSAMPLE][20];
    ...
    for (int isam=0; isam<NSAMPLE; ++isam) {
        for (Int_t ibin=0; ibin<nccqebins; ibin++) {
            ...
            pgrccqemc_veto[isam][ibin] = new TGraph(NCUTFV,xsigma_veto,nevtbyBinmc_veto);
            pgrccqedt_veto[isam][ibin] = new TGraph(NCUTFV,xsigma_veto,nevtbyBindt_veto);
            pgrccqesub_veto[isam][ibin] = new TGraph(NCUTFV,xsigma_veto,nevtbyBinsub_veto);
    ...
    double aVariationTiming[10];
    double aVariationFV[10];
    double aVariationVeto[10];
    double aVariationAll[10];
    ...
    for (int isam=0; isam<NSAMPLE; ++isam) {
        ...
        pUncertaintyTiming[isam] = new TGraph(10,xBinValue,aVariationTiming);
        pUncertaintyTiming[isam]->Write(Form("detresponse_timing_uncertainty_sam%d",isam));
        
        pUncertaintyFV[isam] = new TGraph(10,xBinValue,aVariationFV);
        pUncertaintyFV[isam]->Write(Form("detresponse_fv_uncertainty_sam%d",isam));
        
        pUncertaintyVeto[isam] = new TGraph(10,xBinValue,aVariationVeto);
        ...
        pUncertaintyComb[isam] = new TGraph(10,xBinValue,aVariationAll);
